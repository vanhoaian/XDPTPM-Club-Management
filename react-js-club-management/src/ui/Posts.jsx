...
const Posts = ({
  ...
  return (
    <CardContainer>
      ...
      <Actions>
        {style === "Sự kiện bắt buộc" && (
          <>
            <ActionButton
              active={status === 'Tham_gia'}
              onClick={() => handleStatusChange('Tham_gia')}
            >
              {statusLabels.Tham_gia}
            </ActionButton>
            <ActionButton
              active={status === 'Phép_muộn'}
              onClick={() => handleStatusChange('Phép_muộn')}
            >
              {statusLabels.Phép_muộn}
            </ActionButton>
            <ActionButton
              active={status === 'Phép_nghỉ'}
              onClick={() => handleStatusChange('Phép_nghỉ')}
            >
              {statusLabels.Phép_nghỉ}
            </ActionButton>
          </>
        )}
        {(style === "Sự kiện không bắt buộc" || style === "Sự kiện phải đăng ký") && (
          <>
            <ActionButton
              active={status === 'Tham_gia'}
              onClick={() => handleStatusChange('Tham_gia')}
            ...
            <ActionButton
              active={status === 'Không_tham_gia'}
              onClick={() => handleStatusChange('Không_tham_gia')}
            >
              {statusLabels.Không_tham_gia}
            </ActionButton>
          </>
        )}
      </Actions>
